...
from .conftest import HOW_MANY_SUPERHEROES, HOW_MANY_SUPERHEROES_RESPONSE, HOW_MANY_SUPERHEROES_NO_DFID
...
def test_retrieve_command(capsys, loc, superheroes, api_key, how_many_superheroes_q):
    main(["-l", loc, "-a", api_key, "retrieve", "-df", superheroes, how_many_superheroes_q])
...
def test_retrieve_command_no_dfid(capsys, api_key, how_many_superheroes_q):
    main(["-l", "https://localhost:8000", "-a", api_key, "retrieve", how_many_superheroes_q])
    stdout = capsys.readouterr().out
    assert "6895" in stdout
    assert "callId" in json.loads(stdout)
...
def test_response_command(capsys, loc, superheroes, api_key, how_many_superheroes_q):
    main(["-l", loc, "-a", api_key, "response", "--datafile", superheroes, how_many_superheroes_q])
    stdout = capsys.readouterr().out
    assert "6895" in stdout
